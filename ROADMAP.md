...
- An additional real-ANR fixture is prepared for compilation on GitHub: a registered receiver blocks during a foreground broadcast. The opt-in matrix requires `app_runtime/anr`, preserving console output and all reports. Its runtime behavior is not yet verified; no simulated ANR logs are used.
...
- M1 remains incomplete until the full injected failure matrix and Linux/KVM fixtures are verified. The earlier CLI-only video is not the final demo; per the execution instructions, final video work follows the other feasible phases.
...
- [ ] Define when observation starts. `resolve_app_pid` can retry for approximately ten seconds while examples observe for two seconds; prove the target was actually monitored for the requested interval.
...
- [ ] Verify crash/ANR attribution to the current package and run. In `src/emulator/mod.rs`, collected crash summaries currently affect failure classification only when streaming returned an error. Historical/unrelated log entries must neither hide failures nor create them.
- [ ] Add deterministic launch, reader, timeout, crash, ANR, missing-input and cleanup-failure contracts across JSON/HTML/JUnit/Markdown. Preserve the primary failure when cleanup also fails.
